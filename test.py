...
cleaned_sentences = []
df1 = pd.DataFrame()
# 유사도가 높아도 제거하면 안되는 문장의 키워드
keywords = ["남성용", "여성용", "세 이상"]
for i in range(len(df.effect)):
    cnt = 0
    duplicate = []
    for j in range(len(df.effect)):
        if i != j and df.effect[i] == df.effect[j]:
            ratio = SequenceMatcher(None, df.effect[i], df.effect[j]).ratio()
            if not any(keyword in (df.effect[i], df.effect[j]) for keyword in keywords):
                if ratio >= 0.9:
                    if cnt != 1:        # 처음 한번만 df.effect[i] 추가
                        duplicate.append(df.effect[i])
                        cnt = 1
                    duplicate.append(df.effect[j])
    df1['duplicate'] = duplicate
    df1.duplicate.sample(n=1, replace=True)          # 중복 값 중 랜덤한 데이터 1개만 남김
    pd.concat([df, df1])
df.info()
